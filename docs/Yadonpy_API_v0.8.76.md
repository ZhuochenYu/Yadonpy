...
This document describes the primary script-facing APIs in YadonPy `v0.8.76`. It focuses on stable or intended-to-be-stable entry points that are relevant for study scripts and workflow assembly.
...
From `v0.8.76`, the release no longer ships `yd_moldb.tar`. Reference MolDB species are rebuilt explicitly from the Example 07 CSV inputs rather than auto-imported from a bundled archive.
...
- from `v0.8.74`, ions must be passed explicitly through `ions=[...]`; implicit global ion injection is not part of the supported API
- from `v0.8.74`, iterative retries write `.yadonpy/amorphous_cell_pack_diagnostics.json` when `work_dir` is available
...
Key behaviors in `v0.8.74`:

- species-level artifact resolution and reuse;
- grouped local charge scaling when charge-group metadata exist, and fail-closed export for charged polymers when that metadata are missing;
