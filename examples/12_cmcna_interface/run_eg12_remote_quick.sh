...
export YADONPY_EG12_BULK_LOOPS="${YADONPY_EG12_BULK_LOOPS:-1}"
export YADONPY_EG12_BULK_FINAL_NPT_NS="${YADONPY_EG12_BULK_FINAL_NPT_NS:-0.8}"
if [[ -z "${YADONPY_OMP:-}" ]]; then
    export YADONPY_OMP="16"
fi
...
if [[ $# -eq 0 ]]; then
    python "${SCRIPT_DIR}/run_cmcna_interface.py" --profile quick --stop-after interface_build
else
