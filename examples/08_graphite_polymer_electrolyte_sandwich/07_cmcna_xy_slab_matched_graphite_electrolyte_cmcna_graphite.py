...
"""Example 08-07: CMC-Na xy-slab first, then matched graphite/electrolyte stack.
...
footprint, and only then packs electrolyte and assembles
graphite | electrolyte | CMC-Na | graphite.  Temporary phase gates are kept
during pre-release relaxation and removed from final NVT, so the first final
NVT frame is the electrolyte/CMC interdiffusion t=0.
"""
...
xy_match_tolerance_nm = 0.02  # relaxed CMC slab 与最终石墨 XY 的允许差；超过说明不应直接拼接。
...
if __name__ == "__main__":  # 只在直接运行该脚本时执行主工作流。
    ...
    if pre_equilibrate_cmcna_xy_slab:  # True 时先独立预平衡 CMC-Na slab。
        ...
        cmcna_initial_z_nm = cmcna_initial_volume_nm3 / (graphite_compatible_xy_nm[0] * graphite_compatible_xy_nm[1])  # 固定 CMC-first XY 后所需初始 z。
        cmcna_initial_cell = make_orthorhombic_pack_cell((graphite_compatible_xy_nm[0], graphite_compatible_xy_nm[1], cmcna_initial_z_nm))  # 构造 z-open slab 的初始 AC 盒子。
