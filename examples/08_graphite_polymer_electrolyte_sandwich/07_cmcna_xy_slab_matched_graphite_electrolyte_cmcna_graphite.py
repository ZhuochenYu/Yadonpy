...
def charge_region_specs(charge_uC_cm2: float) -> tuple[FixedChargeRegionSpec, FixedChargeRegionSpec]:  # 生成 CMC-facing / opposite 两个石墨内表面的电荷定义。
    return (  # 返回 tuple，便于 neutral shared stack 用空 tuple，charge case 用这个 tuple。
        FixedChargeRegionSpec(  # 下石墨贴着 CMCNA 的内侧表面。
            layer_name="GRAPHITE_BOTTOM",  # 下石墨层。
            region="top",  # 下石墨的 top face 是 CMC-facing surface。
        ...
        FixedChargeRegionSpec(  # 上石墨内侧表面取等量反号，保持电极对口径。
            layer_name="GRAPHITE_TOP",  # 上石墨层。
            region="bottom",  # 上石墨的 bottom face 是 opposite inner face。
...
electrolyte_active_thickness_nm = env_float("EG08_ELECTROLYTE_ACTIVE_THICKNESS_NM", 15.0)  # 电解液 prepared slab 的 active 厚度；不含 wall padding，默认至少 15 nm。
...
if __name__ == "__main__":  # 只在直接运行该脚本时执行主工作流。
    ...
    electrolyte_plan = estimate_electrolyte_counts_for_slab(  # 按 15 nm active slab 和目标密度反推数量，而不是只按面积缩放。
        ...
        active_thickness_nm=max(15.0, float(electrolyte_active_thickness_nm)),  # 至少 15 nm active 电解液层。
    ...
    electrolyte = MolecularLayerSpec(  # 设置中间变量或可调参数，供后续工作流使用。
        ...
        thickness_nm=float(electrolyte_plan["active_thickness_nm"]),  # prepared slab active 厚度，默认至少 15 nm。
    ...
    stack = LayerStackSpec(  # 设置中间变量或可调参数，供后续工作流使用。
        layers=(graphite_bottom, cmcna, electrolyte, graphite_top),  # 共享 t=0 的层顺序：石墨 | CMCNA | 电解液 | 石墨。
