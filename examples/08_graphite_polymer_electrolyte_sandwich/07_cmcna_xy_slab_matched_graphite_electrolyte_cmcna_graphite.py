...
cmcna_bulk_density_reference_g_cm3 = 1.50  # CMC-Na bulk 参考密度；这里只作为诊断参考，不作为 wall-z-NPT 的硬目标。
cmcna_slab_eq_tmax_K = 450.0  # CMC-Na slab 退火最高温度；DP20 链需要更强的构象重排能力。
cmcna_slab_eq_pmax_bar = 2000.0  # wall-z-NPT 初期高压上限；用于把稀疏 slab 自然压实。
cmcna_slab_eq_ns = 0.50  # 正常温压 wall-z-NPT 基础松弛时长；active density 只做稳定性诊断。
...
if __name__ == "__main__":  # 只在直接运行该脚本时执行主工作流。
    ...
    if pre_equilibrate_cmcna_xy_slab:  # True 时先独立预平衡 CMC-Na slab。
        ...
        cmcna_slab = prepare_cmcna_xy_bulk_slab(  # 新的 CMC-Na 专用入口会完成稀疏 AC、z-wall EQ21、active density/Rg 收敛检查。
            ...
            relaxation=CMCNAXYSlabRelaxationSpec(  # CMC-Na z-open bulk slab 的收敛制备配置。
                ...
                density_mode="wall_z_npt",  # 关键：不指定 CMC slab 目标密度，让 z-only wall NPT 自然收敛。
                coordinate_export_policy="wrapped_xy_z_open",  # 关键：导出给 stack 的 GRO 只 wrap XY，Z 保持开边界。
                target_density_g_cm3=None,  # None 表示 active density 只做 plateau 诊断，不强制等于 bulk 参考值。
                wall_padding_nm=0.40,  # z walls 与 active slab 两侧保留 padding，避免原子贴墙。
                cycles="auto",  # wall-z-NPT 退火轮数；auto 使用温和默认轮数。
                max_cycles=40,  # 极端长盒子最多允许的退火轮数。
                max_z_shrink_per_cycle=0.06,  # 仅供显式 target_active_density 旧模式使用；wall_z_npt 不做几何缩放。
                tmax_K=cmcna_slab_eq_tmax_K,  # wall-z-NPT 退火最高温度。
                pmax_bar=cmcna_slab_eq_pmax_bar,  # wall-z-NPT 初期压力。
                pre_nvt_ns=0.02,  # NPT 前短 NVT，先释放局部坏接触。
                wall_npt_ns=0.05,  # 每轮 wall-z-NPT 的长度；正式制备可升到 0.1 ns。
                hot_nvt_ns=0.01,  # 仅供显式 target_active_density 旧模式使用。
                cool_nvt_ns=0.01,  # 仅供显式 target_active_density 旧模式使用。
                final_relax_ns=cmcna_slab_eq_ns,  # 正常温压 wall-z-NPT 基础松弛时长。
    ...
    cmcna = MolecularLayerSpec(  # 设置中间变量或可调参数，供后续工作流使用。
        ...
        density_target_g_cm3=cmcna_bulk_density_reference_g_cm3,  # 仅作为 bulk 参考密度写入 manifest，不参与 prepared slab 重构。
