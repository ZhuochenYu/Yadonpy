...
cmcna_compression_density_g_cm3 = 0.80  # CMC 膜几何压缩参考密度；只决定 wall-gap 目标，不作为“必须等于”的平衡密度。
cmcna_min_active_density_g_cm3 = 0.80  # CMC 膜最低可接受 active density；低于此值说明仍偏稀疏，不建议拼接。
    ...
    if pre_equilibrate_cmcna_xy_slab:  # True 时先独立预平衡 CMC-Na slab。
        ...
        cmcna_slab = prepare_cmcna_xy_membrane(  # CMC-Na 专用入口会完成稀疏 AC、显式 wall-gap 压缩、active density/Rg 收敛检查。
            ...
            relaxation=CMCNAXYSlabRelaxationSpec(  # CMC-Na z-open bulk slab 的收敛制备配置。
                ...
                target_density_g_cm3=cmcna_compression_density_g_cm3,  # 几何压缩参考密度；只用来反推目标 active z。
                ...
                max_z_shrink_per_cycle=0.06,  # 每轮最多缩短 6%，避免从 0.05 初始密度直接暴力压坏。
