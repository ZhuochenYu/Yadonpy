...
interface_time_series_charge_potential = True  # True 输出带电 graphite/EDL 电荷密度、积分电荷和电势 profile 的 CSV/PNG/MP4。
    ...
    for surface_charge in surface_charge_sweep_uC_cm2:  # 遍历当前工作流中的一组对象或任务。
        ...
        if run_sampling:  # 根据当前状态决定是否进入该分支。
            relax = run_layer_stack_relaxation(  # 运行 layer-stack 松弛和 final NVT 采样。
                ...
                z_compressibility_bar_inv=4.5e-6,  # final z-NPT 使用小有效压缩率，避免带电界面盒子突变。
                z_npt_tau_p_ps=20.0,  # final z-NPT 慢耦合，保留循环压缩得到的稳定结构。
            ...
            interface = analy.interface(  # 设置中间变量或可调参数，供后续工作流使用。
                ...
                time_series_charge_potential=interface_time_series_charge_potential,  # 控制带电 graphite/EDL 电荷-电势时间序列。
            )
            ...
            print(f"[{surface_charge:+.1f} uC/cm2] charge_potential_available = {charge_potential.get('available')}")  # True 表示电荷-电势 CSV/MP4 正常生成。
            print(f"[{surface_charge:+.1f} uC/cm2] charge_potential_outputs = {charge_potential.get('csv') or charge_potential.get('reason')}")  # 打印电荷-电势 CSV 路径或失败原因。
