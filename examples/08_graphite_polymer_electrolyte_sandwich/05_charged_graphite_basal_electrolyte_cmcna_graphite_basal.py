...
interface_time_series_rdf = True  # True 输出 RDF + CN 时间序列动画。
    ...
    for surface_charge in surface_charge_sweep_uC_cm2:  # 遍历当前工作流中的一组对象或任务。
        ...
        if run_sampling:  # 根据当前状态决定是否进入该分支。
            ...
            interface = analy.interface(  # 设置中间变量或可调参数，供后续工作流使用。
                ...
                time_series_rdf=interface_time_series_rdf,  # 设置中间变量或可调参数，供后续工作流使用。
                time_series_concentration=interface_time_series_concentration,  # 设置中间变量或可调参数，供后续工作流使用。
                time_series_angles=interface_time_series_angles,  # 设置中间变量或可调参数，供后续工作流使用。
            )
