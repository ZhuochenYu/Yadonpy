...
interface_time_series_rdf = True  # True 输出全体系 cation RDF/CN 和 graphite-EDL RDF/CN；EDL 只取最强反电荷极性位点。
interface_time_series_concentration = True  # True 输出 z 浓度/密度 profile 时间序列，用来看界面层随时间是否漂移。
interface_time_series_angles = True  # True 输出 graphite-EDL 吸附取向角分布时间序列；纵坐标为角度窗口内分子百分比。
...
if __name__ == "__main__":  # 只在直接运行该脚本时执行主工作流。
    ...
    if run_sampling:  # 根据当前状态决定是否进入该分支。
        ...
        charge_potential = (time_series.get("outputs") or {}).get("charge_potential") or {}  # 提取 graphite/EDL 电荷-电势时间序列输出。
