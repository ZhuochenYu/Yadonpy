...
interface_time_series_rdf = True  # True 输出全体系 cation RDF/CN 和 graphite-near RDF/CN；端面体系也会按石墨近表面层选中心。
interface_time_series_concentration = True  # True 输出 z 浓度/密度 profile 时间序列，用来看端面附近液相重排。
interface_time_series_angles = True  # True 输出吸附取向角分布时间序列；纵坐标是各角度窗口分子百分比。
interface_time_series_charge_potential = True  # True 输出 graphite-near/EDL 电荷密度、积分电荷和电势 profile 的 CSV/PNG/MP4。
...
if __name__ == "__main__":  # 只在直接运行该脚本时执行主工作流。
    ...
    if run_sampling:  # 根据当前状态决定是否进入该分支。
        ...
        interface = analy.interface(  # 设置中间变量或可调参数，供后续工作流使用。
            ...
            time_series_rdf=interface_time_series_rdf,  # 控制全体系 RDF/CN 与 graphite-near RDF/CN 时间序列。
            time_series_concentration=interface_time_series_concentration,  # 控制 z-profile 时间序列和动画。
            time_series_angles=interface_time_series_angles,  # 控制吸附取向角时间序列。
            time_series_charge_potential=interface_time_series_charge_potential,  # 控制 graphite-near/EDL 电荷-电势时间序列。
        )
        ...
        edl_rdf = (time_series.get("outputs") or {}).get("edl_rdf_cn") or {}  # 提取 graphite-near RDF/CN 输出，便于确认新统计是否产出。
        charge_potential = (time_series.get("outputs") or {}).get("charge_potential") or {}  # 提取 graphite-near/EDL 电荷-电势输出。
        ...
        print(f"edl_rdf_cn_available = {edl_rdf.get('available')}")  # True 表示 graphite-near RDF/CN CSV/MP4 正常生成。
