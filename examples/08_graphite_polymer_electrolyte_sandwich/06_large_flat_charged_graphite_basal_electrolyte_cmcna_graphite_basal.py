...
from yadonpy import XYSlabEquilibrationSpec, clean_md_trajectory_files  # 导入本例需要的库或 yadonpy 接口。
...
cmcna_target_slab_density_g_cm3 = 0.50  # CMC-Na xy-slab 目标几何密度；不是最终界面真实密度硬约束。
cmcna_slab_eq_tmax_K = 380.0  # CMC-Na slab 退火最高温度；链太僵可升到 400 K。
cmcna_slab_eq_ns = 0.20  # CMC-Na slab 最终 wall-confined NVT 松弛时长；正式制备可升到 0.5-1 ns。
...
if __name__ == "__main__":  # 只在直接运行该脚本时执行主工作流。
    ...
    graphite_probe = build_graphite(  # 只用于估算石墨 footprint，真正石墨仍由 LayerStackSpec 构建。
        nx=graphite_bottom.nx,  # 使用和下石墨一致的 x 重复数。
        ny=graphite_bottom.ny,  # 使用和下石墨一致的 y 重复数。
        n_layers=graphite_bottom.n_layers,  # 使用和下石墨一致的层数。
        orientation=graphite_bottom.orientation,  # 使用同样的基面/端面设定。
        edge_cap="periodic",  # 与 periodic_xy=True 对齐，估算闭合 XY footprint。
        lateral_margin_ang=10.0,  # 与 layer-stack 默认 graphite margin 保持一致。
        name="GRAPHITE_XY_PROBE",  # probe 只用于尺寸估算，不写入最终 stack。
    )
    graphite_xy_nm = (graphite_probe.box_nm[0], graphite_probe.box_nm[1])  # CMC-Na slab 预平衡使用同一 XY footprint。
    ...
    if pre_equilibrate_cmcna_xy_slab:  # True 时先独立预平衡 CMC-Na slab。
        ...
        cmcna_total_mass_amu = rdkit_mol_mass_amu(CMC) * cmc_chain_count + rdkit_mol_mass_amu(Na) * cmc_chain_count * cmc_dp  # CMC-Na 全原子总质量，单位 amu。
        cmcna_initial_volume_nm3 = (cmcna_total_mass_amu / 6.02214076e23) / cmcna_initial_slab_density_g_cm3 * 1.0e21  # 初始 0.05 g/cm3 对应总体积。
        cmcna_initial_z_nm = cmcna_initial_volume_nm3 / (graphite_xy_nm[0] * graphite_xy_nm[1])  # 固定 XY footprint 后所需初始 z。
        cmcna_initial_cell = make_orthorhombic_pack_cell((graphite_xy_nm[0], graphite_xy_nm[1], cmcna_initial_z_nm))  # 构造 z-open slab 的初始 AC 盒子。
        cmcna_ac = poly.amorphous_cell(  # 先在固定 XY、稀疏 z 中放入 CMC-Na。
            [CMC, Na],  # CMC 链和 Na+ counterion。
            [cmc_chain_count, cmc_chain_count * cmc_dp],  # 8 条 DP20 链和 160 个 Na+。
            cell=cmcna_initial_cell,  # 固定 XY，z 由 0.05 g/cm3 估算。
            density=None,  # 已显式给出 cell，不让 amorphous_cell 再改盒子。
            retry=30,  # 长链稀疏插入仍可能失败，增加重试次数。
            retry_step=2000,  # 每轮 packing 试探步数。
            threshold=2.0,  # 初始排斥阈值；太大更难插入，太小坏接触更多。
            charge_scale=(charge_scale, charge_scale),  # CMC 和 Na+ 同步缩放。
            polyelectrolyte_mode=True,  # 保留 CMC-Na 局域电荷分组信息。
            large_system_mode="large",  # DP20 x 8 使用大体系 packing 策略。
            work_dir=cmcna_slab_dir.child("00_ac"),  # 初始 slab AC 输出目录。
            restart=restart_status,  # 允许断点复用已经构建好的 AC。
        )
        cmcna_slab_eq = eq.EQ21step(cmcna_ac, work_dir=cmcna_slab_dir)  # 创建 CMC-Na slab EQ21 任务。
        cmcna_slab_eq.exec(  # 运行 wall-confined pbc=xy 几何压缩退火。
            temp=temp,  # 目标温度。
            press=1.0,  # 仅作为记录/XY-NPT 可选参数；默认固定 XY，不做 z-NPT。
            mpi=mpi,  # thread-MPI rank。
            omp=omp,  # OpenMP 线程数。
            gpu=gpu,  # GPU 开关。
            gpu_id=gpu_id,  # GPU 编号。
            sim_time=cmcna_slab_eq_ns,  # 兼容 EQ21 参数；xy-slab 分支主要使用 xy_slab.final_relax_ns。
            eq21_tmax=cmcna_slab_eq_tmax_K,  # slab 退火最高温度。
            eq21_dt_ps=0.001,  # 1 fs 步长，优先保证聚电解质 slab 稳定。
            periodicity="xy",  # 关键：z 方向无周期，使用 z walls。
            xy_slab=XYSlabEquilibrationSpec(  # CMC-Na slab 专用配置。
                target_density_g_cm3=cmcna_target_slab_density_g_cm3,  # 目标几何密度约 0.5 g/cm3。
                cycles="auto",  # 自动按每轮最大压缩比例生成 cycles。
                max_cycles=30,  # 防止极端盒子生成过多阶段。
                max_z_shrink_per_cycle=0.10,  # 每轮最多缩短 10% z，避免一次性压坏链构象。
                wall_padding_nm=0.40,  # slab 两侧保留 wall padding，避免原子贴墙。
                xy_area_mode="fixed",  # 固定 XY footprint，确保后续能直接拼进石墨 stack。
                hot_nvt_ns=0.01,  # 每轮热 NVT 时间；正式制备可适当增加。
                cool_nvt_ns=0.01,  # 每轮回到目标温度的 NVT 时间。
                final_relax_ns=cmcna_slab_eq_ns,  # 到目标 z 后的最终 wall-confined NVT。
            ),
            restart=restart_status,  # 允许断点续跑。
        )
        cmcna_prepared_slab_gro = cmcna_slab_eq.final_gro()  # 这个 GRO 将作为 CMCNA layer 的真实初始坐标。
    ...
    if run_sampling:  # 根据当前状态决定是否进入该分支。
        relax = run_layer_stack_relaxation(  # 运行 layer-stack 松弛和 final NVT 采样。
            ...
            z_compressibility_bar_inv=4.5e-6,  # final z-NPT 使用小有效压缩率，避免大扁盒子被瞬时拉伸/压缩。
            z_npt_tau_p_ps=20.0,  # final z-NPT 使用慢压力耦合，确保石墨 restraint 能随盒子平稳缩放。
            ...
            compression_anneal=ZCompressionAnnealSpec(  # 传入循环压缩退火配置。
                ...
                pmax_bar=1500.0,  # z-only 高压上限；大扁盒子先用温和高压，避免 barostat 一步缩放过猛。
