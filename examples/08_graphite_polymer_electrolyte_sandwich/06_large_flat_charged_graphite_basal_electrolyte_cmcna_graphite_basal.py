...
from yadonpy.core import calc, poly, utils, workdir  # 导入本例需要的库或 yadonpy 接口。
...
if __name__ == "__main__":  # 只在直接运行该脚本时执行主工作流。
    ...
    if pre_equilibrate_cmcna_xy_slab:  # True 时先独立预平衡 CMC-Na slab。
        ...
        cmcna_total_mass_amu = calc.molecular_weight(CMC) * cmc_chain_count + calc.molecular_weight(Na) * cmc_chain_count * cmc_dp  # CMC-Na 总质量，单位 amu。
