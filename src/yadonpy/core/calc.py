...
# Conformer MM pre-optimization uses RDKit MMFF (no external MD engine).
...
def _sanitize_qm_kwargs(kwargs: dict, *, psi4_omp=None):
    """Remove/translate non-QM kwargs that would crash QMw backends.

    Users sometimes pass MD-related keys (e.g. mpi/omp) into QM helpers.
    Psi4w does not accept these and will raise TypeError.

    Rules:
      - If kwargs contains "omp" and psi4_omp is unset (-1/None), treat it as psi4_omp.
      - Otherwise, drop "omp" to avoid duplicate values (core.calc passes omp explicitly).
      - Drop common MD keys: mpi, gpu, gmx, gmx_mpi.

    Returns:
        (clean_kwargs, psi4_omp)
    """
    kw = dict(kwargs or {})

    # Legacy/accidental: omp passed via **kwargs
    if 'omp' in kw:
        if psi4_omp in (None, -1):
            try:
                psi4_omp = int(kw.pop('omp'))
            except Exception:
                kw.pop('omp', None)
        else:
            # Prefer explicit psi4_omp; drop duplicate
            kw.pop('omp', None)

    # MD/runtime keys that Psi4w/QMw backends don't accept
    for k in ('mpi', 'gpu', 'gmx', 'gmx_mpi', 'ntmpi', 'ntomp'):
        if k in kw:
            kw.pop(k, None)
...
def _rdkit_mm_optimize(mol: Chem.Mol, conf_id: int, prop=None):
    """Robust MM pre-optimization for a single conformer.

    RDKit may return None for MMFF/UFF forcefields for small/edge molecules
    (e.g. single-atom terminators, missing parameters, unusual valence).

    Returns:
        (energy, coord)
    """

    def _coerce_positions(pos, natoms: int) -> np.ndarray:
        """Convert RDKit forcefield positions into float ndarray (natoms, 3).

        RDKit may return different types depending on version/build:
          - list/tuple of Point3D (len = natoms)
          - flat array-like of length 3*natoms
          - 1D array of Point3D (len = natoms)
          - 2D array-like (natoms, 3)
        """
        if pos is None:
            raise ValueError('positions is None')

        # Fast path: already numeric array
        arr = np.asarray(pos)
        if arr.ndim == 2 and arr.shape == (natoms, 3):
            return arr.astype(float, copy=False)

        # Flat numeric (3N,)
        if arr.ndim == 1 and arr.size == natoms * 3:
            return arr.astype(float, copy=False).reshape(natoms, 3)

        # Point3D list/array
        if arr.ndim == 1 and arr.size == natoms:
            try:
                return np.array([[p.x, p.y, p.z] for p in pos], dtype=float)
            except Exception as e:
                raise ValueError(f'cannot coerce Point3D positions: {e}')

        raise ValueError(f'illegal positions shape {arr.shape} for natoms={natoms}')

    # Trivial molecules: nothing to optimize
    if mol.GetNumAtoms() <= 1 or mol.GetNumBonds() == 0:
        conf = mol.GetConformer(conf_id)
        coord = np.array([[conf.GetAtomPosition(i).x,
    ...
    try:
        ...
        if prop is not None:
            mmff = AllChem.MMFFGetMoleculeForceField(mol, prop, confId=conf_id)
            if mmff is not None:
                mmff.Minimize()
                coord = _coerce_positions(mmff.Positions(), mol.GetNumAtoms())
                return float(mmff.CalcEnergy()), coord
    except Exception:
        pass
    ...
    try:
        ...
        if uff is not None:
            uff.Minimize()
            coord = _coerce_positions(uff.Positions(), mol.GetNumAtoms())
            return float(uff.CalcEnergy()), coord
    except Exception:
        pass

    # 3) Last resort: keep geometry, neutral energy
    conf = mol.GetConformer(conf_id)
    coord = np.array([[conf.GetAtomPosition(i).x,
                       conf.GetAtomPosition(i).y,
                       conf.GetAtomPosition(i).z] for i in range(mol.GetNumAtoms())], dtype=float)
...
# -----------------------------------------------------------------------------
# Optional: GROMACS-based MM refinement for conformers
# -----------------------------------------------------------------------------


def _parse_gro_positions(gro_path: str, natoms: int) -> np.ndarray:
    """Parse positions (nm) from a .gro file and return Angstrom ndarray (natoms,3)."""
    lines = Path(gro_path).read_text(encoding='utf-8', errors='replace').splitlines()
    if len(lines) < 2:
        raise ValueError(f'Invalid .gro file: {gro_path}')
    n = int(lines[1].strip())
    if n != natoms:
        raise ValueError(f'.gro natoms mismatch: expected {natoms}, got {n}')
    coords = []
    for ln in lines[2:2+natoms]:
        # x y z are the last 3 fields (nm)
        parts = ln.split()
        if len(parts) < 3:
            raise ValueError('Malformed .gro coordinate line')
        x, y, z = map(float, parts[-3:])
        coords.append([x*10.0, y*10.0, z*10.0])  # nm -> Å
    return np.array(coords, dtype=float)


def _write_em_mdp(mdp_path: str, *, nsteps: int = 5000, emtol: float = 1000.0) -> None:
    """Write a conservative steepest-descent EM mdp for single-molecule relaxation."""
    mdp = f"""; auto-generated by yadonpy core.calc (conformer refinement)
...
nstlog          = 1
"""
    Path(mdp_path).write_text(mdp, encoding='utf-8')


def _gmx_em_optimize_conformer(
    mol: Chem.Mol,
    conf_id: int,
    *,
    ff=None,
    ...
    """Energy-minimize one conformer with GROMACS and return (E_kcal, coord_angstrom).

    This is optional and intended as a *refinement* step for a small number of low-energy
    conformers. Running it for hundreds/thousands of conformers will be very slow.

    Requirements:
      - mol must have FF properties (ff_type/ff_sigma/ff_epsilon/bonds/angles/dihedrals).
        If `ff` is provided and has `ff_assign`, we attempt to assign first.
      - GROMACS executable available in PATH.

    If anything fails, we raise and let caller fall back to RDKit results.
    """
    from rdkit import Chem as _Chem
    from ..gmx.engine import GromacsRunner
    from ..io.gromacs_molecule import write_gromacs_single_molecule_topology

    wdir = Path(work_dir)
    wdir.mkdir(parents=True, exist_ok=True)

    # Prepare a single-conformer molecule
    mol_i = _Chem.Mol(mol)
    conf = mol.GetConformer(conf_id)
    mol_i.RemoveAllConformers()
    mol_i.AddConformer(_Chem.Conformer(conf), assignId=True)

    # Assign FF if needed
    if ff is not None and hasattr(ff, 'ff_assign'):
        try:
            ff.ff_assign(mol_i)
        except Exception:
            # continue; writer will fail if params are missing
            pass

    runner = gmx_runner or GromacsRunner()

    mol_name = 'MOL'
    gro_path, itp_path, top_path = write_gromacs_single_molecule_topology(mol_i, wdir, mol_name=mol_name)

    boxed_gro = wdir / f"{mol_name}.boxed.gro"
    # Make a reasonable box; -d is nm. Use 1.0 nm padding.
    runner.run(['editconf', '-f', str(gro_path), '-o', str(boxed_gro), '-c', '-d', '1.0', '-bt', 'cubic'], cwd=wdir)

    mdp_path = wdir / 'em.mdp'
    ...
    coord = _parse_gro_positions(str(em_gro), natoms=mol.GetNumAtoms())

    # Potential energy (kJ/mol) -> kcal/mol
    e_kcal = 0.0
    try:
        out_xvg = wdir / 'pot.xvg'
        runner.energy_xvg(edr=em_edr, out_xvg=out_xvg, terms=['Potential'], cwd=wdir)
        lines = [ln for ln in out_xvg.read_text(encoding='utf-8', errors='replace').splitlines() if ln.strip() and not ln.startswith(('#','@'))]
        if lines:
            e_kj = float(lines[-1].split()[1])
            e_kcal = e_kj / 4.184
    except Exception:
        e_kcal = 0.0

    return float(e_kcal), coord
...
def assign_charges(mol, charge='gasteiger', confId=0, opt=True, work_dir=None, tmp_dir=None, log_name='charge', qm_solver='psi4',
    ...
    **kwargs):
    ...
    Optional args:
        ...
        charge_method: Using method in the charge calculation (str, default:wb97m-d3bj)
        charge_basis: Using basis set in the charge calculation (str, default:def2-TZVP)
    ...
    if charge == 'zero':
        ...
        # ------------------------------------------------------------------
        # yzc-gmx-gen inspired hardening
        #
        # - Small inorganic ions (PF6-, BF4-, ClO4- ...) are often poorly handled by
        #   RDKit/MMFF initial geometries. Prefer OpenBabel-based 3D building.
        # - Highly symmetric/polyhedral ions can be problematic for OptKing internal
        #   coordinates; Psi4 recommends OPT_COORDINATES=CARTESIAN for stability.
        # ------------------------------------------------------------------
        smiles_hint = None
        try:
            if mol.HasProp('_yadonpy_input_smiles'):
                smiles_hint = mol.GetProp('_yadonpy_input_smiles')
            elif mol.HasProp('_yadonpy_smiles'):
                smiles_hint = mol.GetProp('_yadonpy_smiles')
        except Exception:
            smiles_hint = None
        ...
        # Ensure we have 3D coordinates; prefer OpenBabel for inorganic ions.
        try:
            if mol.GetNumConformers() == 0 or is_inorganic:
                utils.ensure_3d_coords(mol, smiles_hint=smiles_hint, engine='openbabel' if is_inorganic else 'auto')
        except Exception:
            pass

        # Allow callers to override; otherwise choose robust defaults.
        opt_coordinates = kwargs.pop('opt_coordinates', None)
        dynamic_level = kwargs.pop('dynamic_level', None)
        if is_inorganic and opt_coordinates is None:
            opt_coordinates = 'CARTESIAN'
        if is_inorganic and dynamic_level is None:
            dynamic_level = 1
        try:
            dynamic_level = int(dynamic_level) if dynamic_level is not None else 0
        except Exception:
            dynamic_level = 0
        ...
        utils.radon_print('%s is not implemented in YadonPy.' % (charge), level=3)
        return False
...
def vdw_volume(mol, confId=0, method='grid', radii='rdkit', gridSpacing=0.2):
    ...
    if method == 'grid':
        ...
        for atom in mol.GetAtoms():
            if radii == 'ff':
                r.append((atom.GetDoubleProp('ff_sigma')*10.0) * 2**(1/6) / 2)  # ff_sigma stored in nm
            elif radii == 'rdkit':
...
def conformation_search(mol, ff=None, nconf=1000, dft_nconf=0, etkdg_ver=2, rmsthresh=0.5, tfdthresh=0.02, clustering='TFD',
    ...
    Optional args:
        ...
        mm_mp: Number of parallel execution of RDKit MM optimization (int)
    ...
    # Best-effort SMILES hint (used by robust geometry builders).
    smiles_hint = None
    try:
        if mol_c.HasProp('_yadonpy_input_smiles'):
            smiles_hint = mol_c.GetProp('_yadonpy_input_smiles')
        elif mol_c.HasProp('_yadonpy_smiles'):
            smiles_hint = mol_c.GetProp('_yadonpy_smiles')
    except Exception:
        smiles_hint = None
    ...
    is_inorganic = utils.is_inorganic_ion_like(mol_c, smiles_hint=smiles_hint)
    force_skip_mm = False
    skip_embed = False
    if is_inorganic:
        nconf = 1
        # If caller didn't request DFT refinement explicitly, do at least one DFT opt
        # because RESP benefits from a sensible minimum geometry.
        if dft_nconf <= 0:
            dft_nconf = 1

        # Build a robust 3D geometry with OpenBabel when available.
        try:
            mol_c.RemoveAllConformers()
        except Exception:
            pass
        ok3d = utils.ensure_3d_coords(mol_c, smiles_hint=smiles_hint, engine='openbabel')
        if (not ok3d) or (mol_c.GetNumConformers() == 0):
            utils.radon_print(f'Failed to build 3D coords for inorganic ion: {smiles_hint}', level=3)
            return mol_c, np.array([])
        # Keep this conformer; do not overwrite with RDKit embedding.
        skip_embed = True
        # Skip RDKit MM optimization even if RDKit reports UFF params.
        force_skip_mm = True


    # For tiny molecules (e.g. single-atom terminators), large nconf is pointless
    # and RDKit MMFF/UFF may not be available. Keep it minimal and robust.
    if mol_c.GetNumAtoms() <= 1 or mol_c.GetNumBonds() == 0:
        nconf = 1
        dft_nconf = 0
    ...
    # Users sometimes pass MD-related kwargs (e.g. omp/mpi) into this QM helper.
    # Sanitize them here to prevent TypeError in QMw backends.
    kwargs, psi4_omp = _sanitize_qm_kwargs(kwargs, psi4_omp=psi4_omp)

    if tmp_dir is None: tmp_dir = work_dir

    # Some polyatomic inorganic ions (e.g. PF6-) are not parameterizable by RDKit MMFF/UFF.
    # For those cases we still want a *non-degenerate* initial 3D geometry for QM.
    # Strategy:
    #   1) prefer RDKit distance-geometry embedding with random initial coordinates
    #   2) keep the number of conformers modest (large nconf is pointless here)
    try:
        _has_mmff = bool(AllChem.MMFFHasAllMoleculeParams(mol_c))
    except Exception:
        _has_mmff = False
    try:
        _has_uff = bool(AllChem.UFFHasAllMoleculeParams(mol_c))
    except Exception:
        _has_uff = False
    _need_robust_embed = bool(force_skip_mm) or ((not _has_mmff) and (not _has_uff) and (int(mol_c.GetNumAtoms()) <= 30))
    if _need_robust_embed:
        # Avoid pathological nconf defaults for small ions.
        try:
            nconf = int(min(int(nconf), 50))
        except Exception:
            nconf = 10
    ...
    if _need_robust_embed:
        ...
        try:
            ...
            pass
        # RDKit 2025+ supports EmbedParameters.timeout (prevents rare hangs).
        if hasattr(etkdg, 'timeout'):
            try:
                etkdg.timeout = 10
            except Exception:
                pass
        if hasattr(etkdg, 'maxIterations'):
            try:
                etkdg.maxIterations = max(int(getattr(etkdg, 'maxIterations', 0)), 2000)
            except Exception:
                pass
        elif hasattr(etkdg, 'maxAttempts'):
            try:
                etkdg.maxAttempts = max(int(getattr(etkdg, 'maxAttempts', 0)), 2000)
            except Exception:
                pass
    ...
    if skip_embed:
        conf_ids = [0]
            ...
            conf_ids = AllChem.EmbedMultipleConfs(mol_c, nconf, etkdg)
            if conf_ids is None:
                conf_ids = []
            # If embedding failed (0 conformers) or produced a degenerate geometry, do a final
            # robust fallback to ensure at least one reasonable conformer.
            if mol_c.GetNumConformers() == 0:
                mol_c.RemoveAllConformers()
                utils.ensure_3d_coords(mol_c, smiles_hint=smiles_hint)
            else:
                try:
                    # detect fully-collapsed conformers (all atoms at the same position)
                    bad = False
                    nat = int(mol_c.GetNumAtoms())
                    for cid in list(range(mol_c.GetNumConformers()))[: min(3, mol_c.GetNumConformers())]:
                        xyz = np.array(mol_c.GetConformer(cid).GetPositions(), dtype=float)
                        if xyz.shape[0] != nat or nat < 2:
                            continue
                        min_d2 = np.inf
                        for i in range(nat):
                            dv = xyz[i + 1:] - xyz[i]
    ...
    energies = [None] * nconf  # list of (energy, conf_id)
    dft_energies = []
    e_res = []
    e_dft_res = []

    # Optimization conformers by MM
    mm_solver_norm = (mm_solver or 'rdkit').lower()

    def _gmx_workdir_for(conf_id: int) -> str:
        base = Path(work_dir) if work_dir is not None else (Path(tmp_dir) if tmp_dir is not None else Path('.'))
        return str(base / '_gmx_confsearch' / f'conf_{conf_id:04d}')

    # We'll reuse RDKit MMFF properties where possible.
    prop = AllChem.MMFFGetMoleculeProperties(mol_c)

    def _set_energy(conf_i: int, e_val: float) -> None:
        energies[conf_i] = (float(e_val), conf_i)


    def _safe_set_conf_positions(conf_i: int, coord) -> None:
        """Safely write coordinates into conformer `conf_i`.

        Different RDKit / backend optimizers may return coordinates in different shapes:
          - (natoms, 3)
          - flat (3*natoms,)
          - list/array of Point3D
        If coercion fails, keep the existing conformer geometry (do not raise).
        """
        try:
            natoms = mol_c.GetNumAtoms()
            if coord is None:
                return
            import numpy as _np
            arr = _np.asarray(coord)

            # already (N,3)
            if arr.ndim == 2 and arr.shape == (natoms, 3):
                arr2 = arr.astype(float, copy=False)
            # flat (3N,)
            elif arr.ndim == 1 and arr.size == natoms * 3:
                arr2 = arr.astype(float, copy=False).reshape(natoms, 3)
            # Point3D list/array
            elif arr.ndim == 1 and arr.size == natoms:
                try:
                    arr2 = _np.array([[p.x, p.y, p.z] for p in coord], dtype=float)
                except Exception:
                    arr2 = _np.array([list(p) for p in coord], dtype=float)
                    if arr2.shape != (natoms, 3):
                        return
            else:
                return

            conf = mol_c.GetConformer(conf_i)
            for j in range(natoms):
                conf.SetAtomPosition(j, Geom.Point3D(float(arr2[j, 0]), float(arr2[j, 1]), float(arr2[j, 2])))
        except Exception:
            return

    if _need_robust_embed:
        # No MMFF/UFF parameters available -> skip MM optimization entirely.
        # We keep conformer geometries from embedding and assign zero (tie) energies,
        # then proceed to clustering/DFT (if requested).
        utils.radon_print('Skip MM optimization (no RDKit MMFF/UFF parameters).', level=1)
        for i in range(nconf):
            _set_energy(i, 0.0)

    elif mm_solver_norm in ('gmx', 'gromacs'):
        utils.radon_print('Start optimization of %i conformers by MM level (GROMACS EM).' % nconf, level=1)
        for i in tqdm(range(nconf), desc='[Conformation search: GROMACS EM]', disable=const.tqdm_disable):
            try:
                e, coord = _gmx_em_optimize_conformer(
                    mol_c,
                    i,
                    ff=ff,
                    work_dir=_gmx_workdir_for(i),
                    ntomp=gmx_ntomp,
                    ntmpi=gmx_ntmpi,
                    gpu_id=(str(gmx_gpu_id) if gmx_gpu_id is not None else None),
                )
            except Exception:
                # Fallback to RDKit MMFF/UFF
                e, coord = _rdkit_mm_optimize(mol_c, i, prop=prop)
            _set_energy(i, e)
            _safe_set_conf_positions(i, coord)

    else:
        utils.radon_print('Start optimization of %i conformers by MM level (RDKit MMFF).' % nconf, level=1)
            ...
            if const.mpi4py_avail:
                ...
                with MPIPoolExecutor(max_workers=mm_mp) as executor:
                    ...
                    for i, res in enumerate(results):
                        _set_energy(i, res[0])
                        _safe_set_conf_positions(i, res[1])
        ...
        # Optional: refine the lowest-energy N conformers using GROMACS EM
        try:
            gmx_refine_n_int = int(gmx_refine_n or 0)
        except Exception:
            gmx_refine_n_int = 0
        if gmx_refine_n_int > 0:
            # sort by current MM energy
            ranked = sorted([x for x in energies if x is not None], key=lambda t: t[0])
            ranked = ranked[: min(gmx_refine_n_int, len(ranked))]
            if ranked:
                utils.radon_print('GROMACS refinement of top %i conformers (EM)...' % len(ranked), level=1)
            for _e0, cid in ranked:
                try:
                    e1, coord1 = _gmx_em_optimize_conformer(
                        mol_c,
                        int(cid),
                        ff=ff,
                        work_dir=_gmx_workdir_for(int(cid)),
                        ntomp=gmx_ntomp,
                        ntmpi=gmx_ntmpi,
                        gpu_id=(str(gmx_gpu_id) if gmx_gpu_id is not None else None),
                    )
                    _set_energy(int(cid), e1)
                    _safe_set_conf_positions(int(cid), coord1)
                except Exception:
                    pass
        ...
        # Sequential execution of psi4 optimizations
            ...
            for i in tqdm(range(dft_nconf), desc='[DFT optimization]', disable=const.tqdm_disable):
                ...
                energy, _ = psi4mol.optimize(ignore_conv_error=True, geom_iter=geom_iter, geom_conv=geom_conv, geom_algorithm=geom_algorithm,
                                      opt_coordinates=('CARTESIAN' if is_inorganic else None),
                                      dynamic_level=(1 if is_inorganic else 0))
...
def _conf_search_rdkit_worker(args):
    ...
    energy, coord = _rdkit_mm_optimize(mol, confId, prop=prop)
...
def _conf_search_psi4_worker(args):
    ...
    # Extra safety: avoid passing non-QM kwargs into QMw backends.
    kwargs, psi4_omp = _sanitize_qm_kwargs(kwargs, psi4_omp=psi4_omp)
...
def set_lj_matrix(mol, ff):
    ...
    for i, p1 in enumerate(mol.GetAtoms()):
        for j, p2 in enumerate(mol.GetAtoms()):
            lj_sigma_matrix[i,j] = ((p1.GetDoubleProp('ff_sigma')*10.0) + (p2.GetDoubleProp('ff_sigma')*10.0)) / 2  # nm->Å
            lj_epsilon_matrix[i,j] = sqrt((p1.GetDoubleProp('ff_epsilon')/4.184) * (p2.GetDoubleProp('ff_epsilon')/4.184))  # kJ->kcal
