...
class GAFF2(GAFF):
    """
    gaff2.GAFF2() class (base class: gaff.GAFF)
    ...
    def __init__(self, db_file=None):
        ...
        if type(self) is GAFF2 and getattr(self.param, 'ff_name', None) not in (None, 'gaff2'):
            raise ValueError(f"GAFF2 loaded unexpected ff_name={getattr(self.param, 'ff_name', None)} from {db_file}")
        self.name = 'gaff2'
        ...
        alt_ptype_gaff2 = {
            'hx': 'hc',
            'cs': 'c',
            'ns': 'n', 'nt': 'n', 'nu': 'nh', 'nv': 'nh', 'n7': 'n3', 'n8': 'n3', 'n9': 'n3',
            'nx': 'n4', 'ny': 'n4', 'nz': 'n4', 'n+': 'n4'
        }
...
def mol(
    ...
    *,
    ...
    """Load or build an RDKit Mol from the molecule DB.

    - smiles: canonicalized and keyed by InChIKey-like sha256 over canonical SMILES
    - psmiles (contains '*'): keyed by sha256 over trimmed string

    Geometry (mol2) + charges are stored in a shared DB independent of force-field type.
    """
    ...
    if require_db:
        mol, _ = db.load_mol(smiles_or_psmiles, require_ready=False)
        return mol

    mol, _ = db.build_or_load(smiles_or_psmiles, name=name, prefer_db=prefer_db)
    return mol
...
def chg_assign(
    ...
    **kwargs,
    ...
    """Assign charges to an RDKit Mol and store them into the shared molecule DB."""
    from pathlib import Path
    from ..moldb import MolDB
    from ..core import calc as qm

    method_u = method.upper()
    db = MolDB(Path(db_dir) if db_dir is not None else None)

    if not mol.HasProp("_YADONPY_KEY"):
        raise ValueError(
            "Mol does not have DB key; create it via GAFF2.mol(smiles_or_psmiles, ...) first."
        )
    key = mol.GetProp("_YADONPY_KEY")
    rec = db.load_record(key)
    if rec is None:
        # Should not happen if created via db, but keep robust.
        raise FileNotFoundError(f"Mol record missing in DB for key={key}")

    if (not overwrite) and db.charges_path(key).exists() and rec.ready:
        # Already charged; just make sure attached.
        mol = db.attach_charges(mol, key)
        return mol

    # Charge calculation
    if work_dir is None:
        work_dir = db.record_dir(key) / "charge_calc"
    ...
    # Persist
    db.save_charges(key, mol, method=method_u)
    rec.charge_method = method_u
    rec.ready = True
    db.save_record(rec)
    return mol
