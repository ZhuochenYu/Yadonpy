...
def test_density_plateau_accepts_small_tail_drift_even_when_slope_is_noisy():
    t_ps = np.linspace(0.0, 1000.0, 1001)
    y = 1100.0 + 0.12 * (t_ps - t_ps[0])
    res = find_plateau_start(
        t_ps,
        y,
        min_window_frac=0.2,
    ...
    assert res.ok is True
    assert res.rel_drift < 0.03


def test_density_plateau_rejects_large_tail_drift_with_rel_drift_gate():
    t_ps = np.linspace(0.0, 1000.0, 1001)
    y = 1100.0 + 0.25 * (t_ps - t_ps[0])
    res = find_plateau_start(
        t_ps,
        y,
        min_window_frac=0.2,
