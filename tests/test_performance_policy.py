...
def test_policy_keeps_optional_both_format_trr_coarse_by_default():
    ...
    assert policy.trajectory_format == "xtc_trr"
    assert policy.xtc_ps == pytest.approx(policy.traj_ps)
    assert policy.trr_ps == pytest.approx(50.0)
...
def test_analyzer_auto_profile_reads_production_policy(tmp_path: Path):
    ...
    assert analyzer._analysis_policy_cache_meta()["trajectory_format"] == "trr"


def test_analyzer_uses_trr_when_xtc_is_absent(tmp_path: Path):
