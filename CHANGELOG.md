...
- gmx/workflows/tg.py + gmx/workflows/elongation.py + tests/test_mechanics_studies.py + tests/test_gmx_workflows.py: enrich the mechanics workflow outputs instead of only emitting raw CSV/XVG artifacts. `Tg` summaries now include specific volume, fit metric, split index, and residuals; elongation summaries now include a material-summary block with Young's modulus, peak stress, and strain at peak stress; and both jobs now serialize resource metadata robustly even when optional MPI/OMP counts are unset.
- interface/sandwich.py + interface/sandwich_metrics.py + interface/sandwich_examples.py + tests/test_sandwich_workflow.py: add explicit Example 08 acceptance reporting. Sandwich manifests and progress outputs now carry fixed acceptance fields for phase order, wrapped-across-Z checks, core gaps, and polymer/electrolyte bulk-like density windows, while the shared summary printer remains backward compatible with older result objects.
...
- interface/sandwich + examples/08_graphite_polymer_electrolyte_sandwich/03_cmcna_smoke.py + examples/08_graphite_polymer_electrolyte_sandwich/04_cmcna_full.py + examples/08_graphite_polymer_electrolyte_sandwich/05_cmcna_glucose6_periodic_case.py + tests/test_sandwich_workflow.py: fix the Example 08 phase-density interpretation so confined-slab reports expose the bulk-like center density as the representative phase density while still preserving the full occupied-slab density, compute that core density from atomwise mass inside the central slab window instead of whole-molecule COM selection, raise the CMC-Na target density baseline to `1.50 g/cm^3`, add a molecule-wise Z-compression step for confined rescue rounds so over-dilated electrolyte slabs are actively pushed back toward the target thickness before re-relaxation, and extend stack checks with robust percentile-based core-gap metrics instead of relying only on strict atomwise extrema.
